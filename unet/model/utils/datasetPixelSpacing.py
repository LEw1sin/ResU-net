...
class PixelSpacing_Loader(Dataset):
    ...
    def __getitem__(self, indices):
        ...
        PixelSpacing = dicom_data.PixelSpacing
        return PixelSpacing
...
if __name__ == "__main__":
    isbi_dataset = PixelSpacing_Loader("unet/wly/normal control child/zhuang_xi_tong_014Y")
    ...
    print(isbi_dataset.imgs_path[3])
    for img_index in range(len(isbi_dataset.imgs_path[folder_index])):
        PixelSpacing = isbi_dataset[folder_index, img_index]
        print("获取到的图像维度：", PixelSpacing)
